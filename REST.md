...
En esta dirección se devuelve un String que contiene la ruta donde se encuentra situado Main.html que es el html de la página principal
- GET con ruta => ip:puerto/api/chat
En esta dirección se devuelven los mensajes del chat tiene dos RequestParam, since (id del mensaje desde el que se quieren obtener) y username (nombre de usuario del que ejecuta la petición "esa es la intención", esto con el objetivo de llevar el control de usuarios conectados
- POST con ruta => ip:puerto/api/chat
En esta dirección se sube un mensaje al chat tiene dos RequestParam, message (el mensaje en sí) y username (para saber quien lo envia)
